...
public class FundingEditor extends JPanel implements Configurable {
    ...
    private SchemaModel.Class getEditorSchemaClass(){
        SchemaModel.Class clazz = Service.SCHEMA.getSchemaClassMap(configuration.getSelectedYear()).get(4);
        ...
        for (SchemaModel.Class.Group group : tempGroups){
            if (group.getId().equals(Constants.Schema.CAPITAL_GROUP_ID)) clazz.getGroup().remove(group);
        }
        return clazz;
    }
