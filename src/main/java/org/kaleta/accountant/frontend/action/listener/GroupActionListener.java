 ...
 */
public class GroupActionListener extends ActionListener {
    ...
    public GroupActionListener(Configurable configurable, int action, int cId, int gId) {
        super(configurable);
        this.action = action;
        this.cId = cId;
        this.gId = gId;
    ...
    private void createGroup(String name){
        ...
        switch (cId){
            case 0: {
                ...
                accumulatedDepreciationAccount.setName("Accumulated Depreciation of " + name);
                accumulatedDepreciationAccount.setType("L");
                ...
                depreciationAccount.setName("Depreciation of " + name);
                depreciationAccount.setType("E");
            ...
            case 1: {
                ...
                consumptionAccount.setName("Consumption of " + name);
                consumptionAccount.setType("E");
    ...
    private void renameGroup(String newName){
        ...
        switch (cId){
            case 0: {
                model.getClasses().get(cId).getGroups().get(9).getAccounts().get(gId).setName("Accumulated Depreciation of " + newName);
                model.getClasses().get(5).getGroups().get(9).getAccounts().get(gId).setName("Depreciation of " + newName);
                break;
            }
            case 1: {
                model.getClasses().get(5).getGroups().get(8).getAccounts().get(gId).setName("Consumption of " + newName);
                break;
            }
            // todo impl if needed
        }
